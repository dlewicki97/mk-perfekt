...
class Town_articles extends CI_Controller {
	...
	public function show($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'town_id',  $id);
			echo loadSubViewsBack($this->uri->segment(2), 'index', $data);
		} else {
			redirect('panel');
		}
	...
	public function form($type, $id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
			$data['towns'] = $this->back_m->get_all('towns');

            if($type == 'update') {
			    $data['value'] = $this->back_m->get_one($this->uri->segment(2), $id);
				$data['town_id'] =  $data['value']->town_id;
            }
			elseif($type == 'insert'){
				$data['town_id'] = $id;
			}
	...
	public function action($type, $table, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            ...
            if($type == 'insert') {
				...
				$town_id = $insert['town_id'];
            } else {
			    $this->back_m->update($table, $insert, $id);
			    $this->session->set_flashdata('flashdata', 'Rekord został zaktualizowany!');   
				$town_id = $this->back_m->get_one($table, $id)->town_id;
            }
			redirect('panel/'.$table.'/show/'.$town_id);
		} else {
			redirect('panel');
		}
    }
