...
class Technical_data extends CI_Controller
{
    ...
    function list($id)
    {
        if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            ...
            $data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'sub_product_id', $id);
            $data['sub_product'] = $this->back_m->get_one('sub_products', $id);
    ...
    public function form($type, $sub_product_id, $id = '')
    {
        if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
            $data = loadDefaultData();
            ...
            $data['sub_product'] = $this->back_m->get_one('sub_products', $sub_product_id);
    ...
    public function action($type, $table, $sub_product_id, $id = '')
    {
        if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            $now = date('Y-m-d');
            ...
            redirect("panel/$table/list/$sub_product_id");
        } else {
            redirect('panel');
        }
    }
