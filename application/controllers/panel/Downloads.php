...
class Downloads extends CI_Controller
{
	...
	public function form($type, $id = '')
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			// DEFAULT DATA
			$data = loadDefaultData();
			$this->load->helper('downloads_downloads_categories');
	...
	public function action($type, $table, $id = '')
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			foreach ($_POST as $key => $value) {
				...
				if (!$this->db->field_exists('size', $table)) {
					$this->base_m->create_column($table, 'size');
				}
				...
				} else if ($key == 'downloads_categories') {
					continue;
				} else {
					$insert[$key] = $value;
				}
			...
			$this->load->helper('downloads_downloads_categories');
			save_downloads_categories($last_id);
